...
class Place:
    ...
    def __init__(self, name, exit=None):
        ...
        if self.exit is not None:
            self.exit.entrance = self #当A的出口不为None,让出口指向的B的入口反过来指向A
        # END Problem 2
...
class Ant(Insect):
    ...
    def __init__(self, health=1):
        ...
        self.double_time = 0#用于instance,直接在函数中定义会不断初始化，不能让值累计
...
class ThrowerAnt(Ant):
    ...
    #在lower_bound （3 <=）范围内是shortthrower的范围
    #在upper_bound （>= 5)范围外是longthrower的范围
...
class FireAnt(Ant):
    ...
    #被攻击，对当前位置所有BEE造成伤害（reflected damage)
    #死亡时对当前位置所有蜜蜂造成额外伤害
    def reduce_health(self, amount):
        ...
        #怎么调用health
        #怎么调用当前位置的bee
        #怎么做到在遍历copy——bees的同时改变current_bees
        #
        # BEGIN Problem 5
        "*** YOUR CODE HERE ***"
        ...
        if initial_health <= amount:    
            reflected_damage = amount + self.damage

        if current_bees:
            for bee in list(current_bees):
...
class QueenAnt(ThrowerAnt):
    ...
    def action(self, gamestate):
        ...
        while tmp_place:#为None停止
            if tmp_place.ant:
                if isinstance(tmp_place.ant,ContainerAnt):#是容器
                    tmp_place.ant.double()  
                    if tmp_place.ant.ant_contained:#容器里有蚂蚁
